...
def cutCube(X, center, shape, padd=0): #center is a 3d coord (zyx)
    print("**Cutting cube: center: Z Y X", center, "shape:", shape) # included print statement here
    print('data shape debuggg XxYxZ ',X.shape)
    ...
    print('correct center shape', center)
    
    print('debuggg hlz, hly, hlx', hlz,' ',hly,'  ', hlx)
    ...
    if ((center - np.array([hlz,hly,hlx])) < 0).any() or (
        ...
        cube = X[int(center[0] - hlx):int(center[0] - hlx + shape[0]), int(center[1] - hly):int(center[1] - hly + shape[1]),
               int(center[2] - hlz):int(center[2] - hlz + shape[2])]
        print('CUBE INSIDE CUT_CUBE FUNCTION',cube)
        return np.copy(cube)
